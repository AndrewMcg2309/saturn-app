...
func AwsStart(){
    ...
    let message = Message(
        message: "Start request from iPhone"
    )
...
func AwsStop(){
    let url_start = URL(string: "https://fyrbzl2e5j.execute-api.eu-west-1.amazonaws.com/stopWebsite")!
    let message = Message(
        message: "Stop request from iPhone"
    )
    struct Message: Encodable {
        let message: String
    }
